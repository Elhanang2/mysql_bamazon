...
function supervisormenu(){
    ...
    inquirer.prompt(
        ...
        if(answer.supervisorlist == "View Product Sales by Department"){
            ...
            connection.query(
            "select department_id ,d.department_name , over_head_costs , sum(ifnull(p.product_sales,0))  as product_sales,(sum(ifnull(p.product_sales,0)) - over_head_costs) as total_profit  from products as p  inner join departments as d on  p.department_name =d.department_name  group by department_name order by department_id ASC;",function(err,res){
                ...
                for (var j = 0; j < res.length ; j++){
                    ...
                    console.log(res[j].department_id + colomnspace(15-y) + res[j].department_name +colomnspace(25-w) + "$"+ res[j].over_head_costs + colomnspace(20-z)+(res[j].product_sales).toFixed(2) + colomnspace(14)+ (res[j].total_profit).toFixed(2));
